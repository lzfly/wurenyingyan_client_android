...
public class ModifyCameraAddrActivity extends BaseActivity {
	...
	private String m_CurrSensorIP;
	
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		requestWindowFeature(Window.FEATURE_CUSTOM_TITLE);
		setContentView(R.layout.yingyan_activity_modify_camera_addr);
		...
		ActivityUtils.setTitleBarCaption(R.string.title_modify_camera_addr);
		...
		m_NameEdit = (EditText)findViewById(R.id.camera_new_addr);
		...
		m_CurrSensorIP = intent.getStringExtra("arg1");
		
		m_NameEdit.setText(m_CurrSensorIP);
	}
	...
	View.OnClickListener confirmOnClickListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (newName != null && !TextUtils.isEmpty(newName))
			{
				...
				if (!newName.equals(m_CurrSensorIP))
				{
					...
					if (sourceSensor != null)
					{
						...
						{
							HttpUtils.updateCameraIP(m_CurrSensorAddr, newName, new HttpUtils.HttpResponseListener() {
								...
								public void onDone(boolean succ, String result) {
									...
									if (succ)
									{
										ToastUtils.show(thisObj, R.string.hint_modify_camera_addr_succ);
										
										DeviceInfo sensor = DeviceList.getDevice(m_CurrSensorAddr);
										if (sensor != null)
										{
											sensor.setCameraIP(newName);
										}
										
										Intent data = new Intent();
										data.putExtra("camera_addr", m_CurrSensorAddr);
										data.putExtra("camera_ip", newName);
										setResult(ConstUtils.S_MODIFY_CAMERA_ADDR_ACTIVITY_RESULT_CODE, data);
						...
						{
							getWaitingDialog().dismiss();
							ToastUtils.show(thisObj, R.string.err_modify_camera_addr);
						}
					}
					else
					{
			...
			{
				ToastUtils.show(thisObj, R.string.please_input_new_camera_addr);
			}
		}
	...
	protected void onStop() {
		...
		SPConfig.setPropery("last_page", "ModifyCameraAddrActivity");
